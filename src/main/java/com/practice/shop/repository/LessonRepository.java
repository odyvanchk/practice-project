...
public interface LessonRepository extends JpaRepository<Lesson, Long> {
    ...
    @Query("select count(l) from Lesson l where l.student.id = ?1 and l.dateTime < now()")
    long countPastByIdStudent(Long idStudent);

    @Query("select count(l) from Lesson l where l.student.id = ?1 and l.dateTime > now()")
    long countFutureByIdStudent(Long idStudent);

    @Query("select count(l) from Lesson l where l.teacher.id = ?1 and l.dateTime < now()")
    long countPastByIdTeacher(Long idTeacher);

    @Query("select count(l) from Lesson l where l.teacher.id = ?1 and l.dateTime > now()")
    long countFutureByIdTeacher(Long idTeacher);

    boolean existsByIdAndStudentId(Long id, Long idStudent);

    boolean existsByTeacherIdAndId(Long idTeacher, Long id);

    @Query("select l from Lesson l where l.teacher.id = ?1 and l.dateTime > now()")
    ...
    @Query("select l from Lesson l where l.student.id = ?1 and l.dateTime < now()")
